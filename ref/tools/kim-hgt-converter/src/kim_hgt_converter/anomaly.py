...
def compute_local_anomaly(
    ...
    *,
    ...
    """Compute a smooth local anomaly field in meters.

    The background follows the original shader's surrounding-sample idea, but is
    evaluated once at pixel centers with bilinear sampling. A light 3x3
    separable smoothing pass removes single-pixel contour noise without changing
    the broad anomaly pattern.
    """
